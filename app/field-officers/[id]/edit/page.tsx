...
export default function EditFieldOfficerPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        const res = await fetch(apiUrl(`/field-officers/${officerId}`), {
        ...
        const assignments = (data.field_officer_points as Array<{ point_id: string }>) || [];
        const assignedPointIds = assignments.map((a) => a.point_id);
        ...
        const timer = setTimeout(() => {
          ...
          setForm((f) => ({ ...f, shiftId: matchShift?.id || '' }));
        }, 500);

        return () => clearTimeout(timer);
      } catch {
        setFormMsg({ type: 'error', text: 'Failed to load field officer data.' });
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        router.push(`/field-officers/${officerId}`);
      } else {
        const j = await res.json().catch(() => ({}));
  ...
  if (loadingOfficer) return <ProtectedRoute allowedRoles={['owner', 'admin']}><Layout><LoadingSpinner /></Layout></ProtectedRoute>;
      ...
      <Layout>
        <div className="mx-auto max-w-3xl space-y-5">
          <PageHeader
            ...
            subtitle={`ID ${officerId.slice(0, 8)}`}
            actions={
              <Button variant="outline" size="sm" type="button" onClick={() => router.push(`/field-officers/${officerId}`)}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  ...
                  <select
                    ...
                    className="h-10 w-full rounded-xl border border-slate-200 bg-white/80 px-3 text-sm text-slate-700 shadow-sm outline-none transition focus:border-sky-400 focus:ring-2 focus:ring-sky-100"
                  >
                    <option value="">Not linked yet</option>
                    {fieldOfficerUsers.map((u) => (
                  ...
                  <p className="mt-1 text-xs text-slate-400">
                    Link a role user so this field officer can log in and see only assigned points.
                  </p>
                </div>
              </div>
            </GlassCard>
            ...
            <div className="flex gap-3">
              ...
              <Button type="button" variant="secondary" onClick={() => router.push(`/field-officers/${officerId}`)}>
                Cancel
              </Button>
            </div>
          </form>
