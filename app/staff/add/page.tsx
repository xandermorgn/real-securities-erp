...
// Merge API list with owner defaults (by lowercase name, prefer API row)
...
export default function AddStaffPage() {
  ...
  const [form, setForm] = useState({
    ...
    accountHolderName: '',
    branch: '',
    advances: [] as { amount: string; date: string; remarks: string }[],
    loginId: '',
    loginPassword: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (newStaffId && form.advances.length > 0) {
          const validAdvances = form.advances.filter((a) => a.amount && a.date);
          for (const adv of validAdvances) {
            await fetch(apiUrl(`/staff/${newStaffId}/advances`), {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) },
              body: JSON.stringify({ amount: parseFloat(adv.amount) || 0, date: adv.date, remarks: adv.remarks || null }),
            }).catch(() => {});
          }
        }
        router.push('/staff');
      ...
      <Layout>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-3 sm:col-span-2">
                ...
                {points.length === 0 && (
                  <p className="text-[11px] text-slate-400">
                    No points yet.{' '}
                    <Link href="/points" className="font-medium text-sky-600 underline underline-offset-2">
                      Add one →
                    </Link>
                  </p>
                )}
          ...
          <GlassCard className="p-6">
            ...
            {(!form.advances || form.advances.length === 0) ? (
              ...
              <div className="space-y-3">
                {form.advances.map((adv, idx) => (
                  <div key={idx} className="grid gap-3 rounded-2xl border border-slate-100 bg-slate-50/60 p-3 sm:grid-cols-[1fr_1fr_1fr_auto]">
                    <Input
                      label="Amount"
                      type="number"
                      placeholder="0"
          ...
          <GlassCard className="p-6">
            ...
            <p className="mb-4 text-xs text-slate-400">
              Auto-generated from first name &amp; phone if left blank. Override to set custom credentials.
            </p>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Input label="User ID" placeholder="Auto: first name (lowercase)" value={form.loginId} onChange={setField('loginId')} />
              <Input label="Password" placeholder="Auto: phone number" value={form.loginPassword} onChange={setField('loginPassword')} />
            </div>
          </GlassCard>
